...
import { filters, singlefilters } from './FiltersData'
...
export default function Product() {
  const [mobileFiltersOpen, setMobileFiltersOpen] = useState(false)
   ...
   const handleFilter = (value, sectionId) => {
    ...
    if (filterValues.length>0 && filterValues[0].split(',').includes(value)) {
      ...
      console.log("includes","",value,sectionId,filterValues)
    }
     else {
  ...
  if(filterValues.length > 0){
    ...
    // Update the URL and maintain history state
             ...
             {singlefilters.map((section) => (
              ...
              <div className="lg:col-span-4 w-full">
                <div className='flex flex-wrap justify-center py-5 bg-white '> 
                    {mens_kurta.map((item)=><ProductCard product={item} key={item}/>)}
                </div>
              </div>
            </div>
          </section>
